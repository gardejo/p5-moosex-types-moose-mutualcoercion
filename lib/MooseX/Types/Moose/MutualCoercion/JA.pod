...
この文書はL<MooseX::Types::Moose::MutualCoercion|MooseX::Types::Moose::MutualCoercion>のバージョンC<0.00>について記述しています。
...
L<Moose|Moose>でよく知られている組み込みまたは一般的な型を相互に型変換するために設計されています。
...
C<< coerce >>を有効にした場合、L<MooseX::Types::Common::String|MooseX::Types::Common::String>由来のC<< NonEmptySimpleStr >>はL<< Class::MOP::load_class()|Class::MOP >>によってロードされることが保証されます。
...
例えば、C<< [foo 0 bar 1] >>はC<< {foo => 0, bar => 1} >>に変換されます。

=item C<< ArrayRefToHashKeys >>

C<< HashRef >>のサブタイプです。
C<< coerce >>を有効にした場合、C<< ArrayRef >>の全ての要素はハッシュリファレンスのキーに代入されます。
例えば、C<< [foo bar baz] >>はC<< {foo => undef, bar => undef, baz => undef} >>に変換されます。
